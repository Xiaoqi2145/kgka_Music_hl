...
| 网易云 / 本地音乐 | `_hydrateLocalLoudnessOnce` 直接 return，不写响度 | `player_controller.dart:790-792` |
...
| DL-10 | `DownloadController.download` 用 `_api.songUrl(song, quality)` 取地址，但 5 个下载入口（`lib/ui/pages/search_page.dart:964-976`、`home_page.dart:1029-1041`、`cloud_drive_page.dart:567-579`、`playlist_detail_page.dart:1606-1618`、`playback_history_page.dart:364-376`）**都不按 `song.source` 过滤** | `lib/controllers/download_controller.dart:334` | 网易云歌曲 `hash = ne_<id>`，`/song/url` 取不到地址必然失败；云盘歌曲应用 `cloudSongUrl`（`lib/controllers/player_controller.dart:847-860`）却仍走 `songUrl`，能否成功**待核实**；本地音乐同样不适用 |
